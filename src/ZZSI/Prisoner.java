...
public class Prisoner {
    ...
    public static Prisoner getRandomPrisoner(Collection<Prisoner> prisonersToChoose, double maxScore) {
        Random rand = new Random();

        if (prisonersToChoose.size() > 0) {

            while (true) {
                ArrayList<Prisoner> list = new ArrayList<Prisoner>(prisonersToChoose);
                Prisoner prisoner = list.get(rand.nextInt(prisonersToChoose.size()));
    ...
    public static LinkedHashMap<String, Prisoner> getCrossedPrisoner(Prisoner father, Prisoner mother, int currentPopulation) {
        Random random = new Random();

        LinkedHashMap<String, Prisoner> result = new LinkedHashMap<String, Prisoner>();
        ...
        result.put(firstChild.getName(), firstChild);
        result.put(secondChild.getName(), secondChild);
        return result;
    }
